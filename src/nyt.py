def nyt():
    ...
    def get_data():
        ...
        df_ny = (
            df_covid[
                (df_covid.state == "New York")
                | (df_covid.state == "New Jersey")
                | (df_covid.state == "Connecticut")
                | (df_covid.state == "Pennsylvania")
                | (df_covid.state == "Massachusetts")
            ]
            .reset_index()
            .drop("index", axis=1)
        )
        ...
        ny_counties = list(
            df_ny[df_ny.date == df_ny.date.max()]
    ...
    def plot_cases(df, counties, date, rolling=3, col="cases"):
        ...
        for i in counties:
            y = (
                df.loc[(df.uid == i) & (df.index <= date), f"{col}_norm_log10"]
        ...
        fig.add_shape(
            ...
            xref="x",
            yref="y",
            x0="2020-03-16 00:00:00",
            x1="2020-03-16 00:00:00",
            y0=-2,
            y1=4,
    ...
    def plot_log_cases(df, counties, date, rolling=3, col="cases"):
        ...
        fig.update_layout(
            template="plotly_white",
            title=go.layout.Title(
                text=f"{col} Growth Rate with {rolling} Day Rolling Average".title()
            ),
    ...
    def plot_log_acceleration(df, counties, date, rolling, col="cases"):
        ...
        for i in counties:
            ...
            slope = [
                (y[1] - y[0]) / (x[1] - x[0])
                for x, y in zip(zip(x, x[1:]), zip(y, y[1:]))
            ]
            fig.add_trace(
                go.Scatter(
        ...
        fig.update_layout(
            template="plotly_white",
            title=go.layout.Title(
                text=f"{col} Growth Acceleration with {rolling} Day Rolling Average".title()
            ),
            xaxis=dict(title=f"total {col} per 100,000 (log10 scale)"),
    ...
    top_counties = list(
        df_covid.loc[df_covid.index == df_covid.index.max(), :]
        .sort_values("cases", ascending=False)
        .uid
    )
    ...
    st.write(
        f"Data Source: https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv"
    )
